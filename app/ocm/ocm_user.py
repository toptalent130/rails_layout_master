...
# os.chdir("E:\\Python\\ocm_project\\app\\ocm\\")
# html.Script(id='js1', src='http://code.jquery.com/jquery-1.11.1.min.js')
...
owner_card = dbc.Card(
    [
        dbc.CardHeader(
            dbc.Tabs(
                [
                    dbc.Tab(label=name, tab_id=name) for name in [row['Name'] for index, row in loadcsvtodf(managers_directory, 'managers.csv').iterrows()]
                ],
                id="tabs-owner",
                card=True,
                active_tab="admin",
...
def update_figure(update, project, apply, checklistiv, checklistrank, dayaction, owner, iv_ec_color_dict, iv_ec_dict, expiration, quote_date, qdseq, ideas):
  ...
  return [tableTradeActionDates, tableTradesList, tableDayActions, tableTradeStatusOverview, fig_ideas, fig]

if __name__ == "__main__":
  # app.run_server(mode='inline', debug=False, use_reloader=False) #jupyter run server
  app.run_server(port=8050, debug=True, use_reloader=False) #dash run server