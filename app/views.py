...
@login_required(login_url="/login/")
def index(request):
    path="/home/ubuntu/stock-management-django/app/ocm_data/TW/SPX/"  # insert the path to your directory   
    ...
    current_user.to_csv('/home/ubuntu/stock-management-django/app/ocm_data/TradeAccounts/current_user.csv', mode='w', header=True, index=False)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if len(TradeAccount.objects.filter(value=value)) == 0:
                ...
                trade_accounts.to_csv('/home/ubuntu/stock-management-django/app/ocm_data/TradeAccounts/trade_accounts.csv', mode='w', header=True, index=False)
                return redirect("/")
            else:
                success = False
